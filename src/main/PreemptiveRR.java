...
public class PreemptiveRR {
    ...
    public static void schedule(LinkedList<Process> processes, int num_IO, int quantum) {
        PriorityQueue<Process> waitingProcessQueue = new PriorityQueue<>(cmp1);
        LinkedList<Process> readyProcessQueue = new LinkedList<>();
        ...
        while (!waitingProcessQueue.isEmpty() || !readyProcessQueue.isEmpty()) {
            ...
            Process curProcess = readyProcessQueue.removeFirst();  //from ready_queue to running
            ...
            if (curProcess.getNextBurstTime() > quantum) {
                curProcess.updateNextBurstTime(-quantum);
                timeNow += quantum;
                curProcess.return_time = timeNow;
                waitingProcessQueue.add(curProcess);
                continue;
            }
            // current process execution time is smaller than quantum
        ...
        System.out.println("Scheduling algorithm: Preemptive Round-Robin Scheduling");
